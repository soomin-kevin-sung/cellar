...
async fn production_restart_recovers_a_renamed_upload_before_readiness() {
    ...
    let verified = adapter.verify_and_close(session.id, 3).await.unwrap();
    let operations = SqliteOperationRepository::new(pool.clone());
    let intent = match operations
        .prepare_upload_commit(session.id, verified, now)
        .await
        .unwrap()
    {
    ...
    adapter.publish_no_replace(&intent).await.unwrap();
    drop(service);
    drop(adapter);
