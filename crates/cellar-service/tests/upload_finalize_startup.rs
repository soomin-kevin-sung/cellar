...
#[tokio::test]
async fn production_fresh_finalize_publishes_with_the_retained_source_handle() {
    ...
    sqlx::query(
        "INSERT INTO project
         ...
         VALUES (?, 'fresh-production', '', 'active', 1,
    ...
    let service =
        initialize_upload_finalization_recovery(&pool, adapter.clone(), adapter, &readiness, now)
            .await
            .unwrap();
    ...
    let session = service
        .create(
            NewUpload {
                ...
                destination_name: "fresh.bin".into(),
                expected_size: 3,
    ...
    let entry = service.finalize(session.id, now).await.unwrap();

    assert_eq!(entry.exact_name.as_str(), "fresh.bin");
    assert_eq!(
        std::fs::read(
            storage_root
                .path()
